...
/* void BigN_to_int(struct BigN *res, struct BigN x)
{
    ...
    unsigned long long max_first = ULLONG_MAX / max10;
    unsigned long long max_mod = ULLONG_MAX - max_first * max10;
...
static inline void subBigN(struct BigN *out, struct BigN x, struct BigN y)
{
    ...
    if (y.lower > x.lower) {
        x.upper--;
        out->lower = ULLONG_MAX - y.lower + x.lower;
    } else {
        out->lower = x.lower - y.lower;
    }
}
...
static inline void multiBigN(struct BigN *out, struct BigN x, struct BigN y)
{
    ...
    // printk("Mult %llu %llu = %llu * %llu",out->upper, out->lower, x.lower,
    // y.lower);
    /* struct BigN res;
    res.upper = 0;
    res.lower = 0;
    BigN_to_int( &res, *out );
    if (res.upper)
        printk("%llu %llu = %llu * %llu", res.upper, res.lower, x.lower,
    y.lower);
    else printk("%llu = %llu * %llu",res.lower, x.lower, y.lower); */
}
